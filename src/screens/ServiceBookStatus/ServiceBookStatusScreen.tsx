...
const ServiceBookStatusScreen = () => {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            const vendorID = showAll ? '0' : selectedVendorId;
    ...
    const getFilteredList = (): any[] => {
        let base = activeTab === 'Pending' ? pendingList : activeTab === 'Hold' ? holdList : recList;
    ...
    const handleMarkReceived = async (item: any) => {
        console.log('[ServiceBookStatus] Mark Received pressed for:', item.Name, '| Chassis:', item.ChassisNumber);
        HelperService.showConfirm(
            ...
            async () => {
                setIsLoading(true);
                try {
                    const res = await ServiceBookRecByRMForRM(item.SalesletterStats_ID, sessionToken!);
                    if (res.IsSuccess) {
                        setPendingList(prev => prev.filter(d => d.SalesletterStats_ID !== item.SalesletterStats_ID));
    ...
    const renderItem = ({ item }: { item: any }) => (
        <View style={styles.card}>
            ...
            <Text style={styles.detail}>{item.ProductName}, {item.VehicleColor}</Text>
            {activeTab === 'Pending' && (
                <TouchableOpacity style={styles.recBtn} onPress={() => handleMarkReceived(item)}>
    ...
    return (
        <View style={styles.container}>
            <View style={styles.tabRow}>
                {(['Rec_ServiceBook', 'Pending', 'Hold'] as TabType[]).map(tab => (
                    <TouchableOpacity
                        key={tab}
                        style={[styles.tabBtn, activeTab === tab && styles.tabBtnActive]}
                        onPress={() => {
                            console.log('[ServiceBookStatus] Tab switched to:', tab);
                            setActiveTab(tab); setSearchText('');
                        }}>
...
const styles = StyleSheet.create({
    ...
    searchRow: { padding: 10, backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#eee' },
    searchInput: { backgroundColor: '#f0f0f0', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8, fontSize: 14, color: '#222' },
    ...
    recBtn: { backgroundColor: '#2dd36f', borderRadius: 6, paddingVertical: 8, alignItems: 'center', marginTop: 8 },
