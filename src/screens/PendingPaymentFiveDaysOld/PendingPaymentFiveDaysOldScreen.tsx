...
const PendingPaymentFiveDaysOldScreen = () => {
    ...
    useFocusEffect(
        useCallback(() => {
            if (!sessionToken || designation !== 'ASM') { return; }
            setIsLoading(true);
            getPaymentPendingMorethan5daysofVendorForRM(selectedRMId, sessionToken)
                .then(res => {
                    if (res.IsSuccess) { setList(res.Data); }
                    else { HelperService.showAlert('Error', res.Msg); }
                })
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={styles.card}>
                            ...
                            <Text style={styles.detail}>Pending Amount: {item.TotalAmt}</Text>
                        </View>
                    )}
                    ListEmptyComponent={
