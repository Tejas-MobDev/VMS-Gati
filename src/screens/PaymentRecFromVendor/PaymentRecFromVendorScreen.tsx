...
const PaymentRecFromVendorScreen = () => {
    ...
    const [hypothicationList, setHypothicationList] = useState<any[]>([]);
    ...
    const [show, setShow] = useState({
        ...
        CustomerName: true, FinancerName: true, DSAName: true,
    });
    ...
    const onCompanySelect = (company: any) => {
        console.log('[PaymentRecFromVendor] Company selected:', company.CompanyName, '| CompanyID:', company.CompanyID);
        // Deep copy to avoid mutating master data
        const vehicles = JSON.parse(JSON.stringify(company.PaymentPendingList ?? []));
        ...
        const epayments = rawEpayments.map((ep: any) => {
            if (ep.TotalEpaymentRecAmt == null) {
                if (
                    ['19', '20', '21'].includes(ep.SalesType) &&
                    (ep.Receivable_HpAmount != null || ep.Receivable_InsuranceCoverAmt != null)
                ) {
                    ep.TotalEpaymentRecAmt =
                        (ep.Receivable_HpAmount ?? 0) + (ep.Receivable_InsuranceCoverAmt ?? 0);
                    if (!ep.PaidTillNow) { ep.PaidTillNow = 0; }
                    ep.Balance =
                        parseInt(ep.TotalEpaymentRecAmt) - parseInt(ep.PaidTillNow);
                } else {
                    ep.TotalEpaymentRecAmt = '';
                    ep.Balance = '';
                }
        ...
        setVehicleList(vehicles.map((v: any) => ({ ...v, IsChecked: false, PaymentRec_AfterPostBack: null })));
    ...
    const recalcTotal = (vehicles: any[], epayments: any[], advance: number) => {
    ...
    const toggleVehicle = (idx: number, checked: boolean) => {
        console.log('[PaymentRecFromVendor] Vehicle toggled | idx:', idx, '| checked:', checked);
        const updated = [...vehicleList];
        updated[idx].IsChecked = checked;
        if (!checked) { updated[idx].PaymentRec_AfterPostBack = null; }
    ...
    const setVehicleAmount = (idx: number, val: string) => {
        console.log('[PaymentRecFromVendor] Vehicle amount changed | idx:', idx, '| val:', val);
        const updated = [...vehicleList];
        const num = parseFloat(val) || 0;
        if (num > updated[idx].Balance) {
            HelperService.showAlert('Error', `Balance amount is Rs.${updated[idx].Balance}`);
    ...
    const toggleEpayment = (idx: number, checked: boolean) => {
        console.log('[PaymentRecFromVendor] Epayment toggled | idx:', idx, '| checked:', checked);
    ...
    const setEpaymentAmount = (idx: number, val: string) => {
        const updated = [...epaymentList];
        const num = parseFloat(val) || 0;
        const hasLimit = updated[idx].TotalEpaymentRecAmt !== '' && updated[idx].TotalEpaymentRecAmt != null;
        if (hasLimit && num > updated[idx].Balance) {
            HelperService.showAlert('Error', `Balance amount is Rs.${updated[idx].Balance}`);
            updated[idx].PaymentRec_AfterPostBack = null;
            updated[idx].IsChecked = false;
        } else {
    ...
    const onPaymentModeChange = async (modeId: string) => {
        ...
        const mode = paymentModeList.find(m => m.ID.toString() === modeId.toString());
        setSelectedPaymentModeText(mode?.Name ?? '');
        // Clear bank details
        setForm(prev => ({
            ...prev,
            PaymentMode: modeId,
            ChqNEFTOfBankNameMasterID: '', Branch_No: '', AccType: '',
            IFSCCode: '', AccHolderName: '', ChqNeft_Number: '', ChequeDate: '',
            SalesLetter_Customer_Name: '', DSABankMasterID: '', HypothicationMasterID: '',
        ...
        if (id === 33 || id === 34) {
            setShow({ AccountNo: true, ChqNo: true, ChqDt: true, BankName: true, BranchNo: true, AccountType: true, IFSC: true, AccHolderName: true, CustomerName: false, FinancerName: false, DSAName: false });
            // Fetch bank details
            if (sessionToken) {
            ...
            setShow({ AccountNo: true, ChqNo: true, ChqDt: true, BankName: true, BranchNo: true, AccountType: true, IFSC: true, AccHolderName: false, CustomerName: true, FinancerName: true, DSAName: true });
        } else if (id === 35) {
            setShow({ AccountNo: false, ChqNo: false, ChqDt: true, BankName: false, BranchNo: false, AccountType: false, IFSC: false, AccHolderName: false, CustomerName: false, FinancerName: false, DSAName: false });
        } else if (id === 4097 || id === 4098) {
            setShow({ AccountNo: true, ChqNo: true, ChqDt: true, BankName: true, BranchNo: true, AccountType: true, IFSC: true, AccHolderName: true, CustomerName: true, FinancerName: false, DSAName: false });
        } else if (id === 5100) {
            setShow({ AccountNo: false, ChqNo: false, ChqDt: true, BankName: false, BranchNo: false, AccountType: false, IFSC: false, AccHolderName: false, CustomerName: true, FinancerName: false, DSAName: false });
        }
    };

    const validateAndNext = async () => {
        console.log('[PaymentRecFromVendor] Next button pressed at step 2 | form:', JSON.stringify(form));
        const f = form;
        const mode = parseInt(f.PaymentMode, 10);

        if (autoCalcTotal !== (parseFloat(f.RecievedAmt) || 0)) {
            HelperService.showAlert('Error', 'Received money is not equal to auto calculated amount.');
            return;
        }
        if (f.AdvanceRecieved && parseFloat(f.AdvanceRecieved) !== 0) {
            ...
            if ((mode === 33 || mode === 600 || mode === 4097) && f.ChqNeft_Number && f.ChqNeft_Number.length !== 6) {
                HelperService.showAlert('Error', 'Cheque number should be 6 digit.');
                return;
            }
            if (!f.PaymentMode || !f.RecievedAmt || !f.PaymentRecInCompany) {
                HelperService.showAlert('Error', 'Please select payment mode and received amount and cheque in name of.');
                return;
            }
            // Mode-specific validation
            const acNo = f.AccountNumberForDD || f.AccountNumberForText;
            if (mode === 33 || mode === 34) {
                if (!acNo || !f.ChqNeft_Number || !f.ChequeDate || !f.ChqNEFTOfBankNameMasterID || !f.Branch_No || !f.AccType || !f.IFSCCode || !f.AccHolderName) {
                    HelperService.showAlert('Error', 'Please enter all Details.');
                    return;
                }
            } else if (mode === 35) {
                if (!f.ChequeDate) { HelperService.showAlert('Error', 'Please enter all Details.'); return; }
            } else if (mode === 600) {
                if (!f.HypothicationMasterID || !f.DSABankMasterID || !acNo || !f.ChqNeft_Number || !f.ChequeDate || !f.ChqNEFTOfBankNameMasterID || !f.Branch_No || !f.AccType || !f.IFSCCode || !f.SalesLetter_Customer_Name) {
                    HelperService.showAlert('Error', 'Please enter all Details.'); return;
                }
            } else if (mode === 601) {
                if (!f.HypothicationMasterID || !f.DSABankMasterID || !acNo || !f.ChqNeft_Number || !f.ChequeDate || !f.ChqNEFTOfBankNameMasterID || !f.SalesLetter_Customer_Name) {
                    HelperService.showAlert('Error', 'Please enter financer, DSA, Account no, ref id, payment date, bank name, customer name Details.'); return;
                }
            } else if (mode === 4097) {
                if (!acNo || !f.ChqNeft_Number || !f.ChequeDate || !f.ChqNEFTOfBankNameMasterID || !f.Branch_No || !f.AccType || !f.IFSCCode || !f.AccHolderName || !f.SalesLetter_Customer_Name) {
                    HelperService.showAlert('Error', 'Please enter all Details.'); return;
                }
            } else if (mode === 4098) {
                if (!f.AccHolderName || !f.ChqNeft_Number || !f.ChequeDate || !f.SalesLetter_Customer_Name) {
                    HelperService.showAlert('Error', 'Please enter ref id, payment date, customer Details.'); return;
                }
            } else if (mode === 5100) {
        ...
        if (f.ChqNeft_Number && f.PaymentMode) {
            ...
            try {
                const chkRes = await IsChequOrPaymentNumberAvail(
                    sessionToken!, f.PaymentMode, f.VendorID, f.Acc_No,
                    f.ChqNEFTOfBankNameMasterID, f.ChqNeft_Number,
                );
                if (chkRes.IsSuccess && chkRes.Data === true) {
                    HelperService.showAlert('Error', 'Same payment details already exist.');
                    setIsLoading(false);
                    return;
                }
            } catch { }
        ...
        if (form.DirectPaymentToAuth) {
            ...
            try {
                const vendorRes = await CheckPurchaseVendorFromSalesOrderDetID(salesOrderDetIds, sessionToken!);
    ...
    const handleSubmit = async () => {
        console.log('[PaymentRecFromVendor] Submit pressed | payload form:', JSON.stringify(form));
        setIsLoading(true);
        const f = form;
        ...
        return (
            <View style={styles.container}>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <Text style={styles.emptyText}>No companies with pending payment.</Text>
                    }
    ...
    if (step === 1) {
        return (
            <ScrollView style={styles.container} contentContainerStyle={{ padding: 12 }}>
                <Text style={styles.sectionTitle}>Vehicle Payments</Text>
    ...
    if (step === 2) {
        return (
            <ScrollView style={styles.container} contentContainerStyle={styles.formContent}>
                ...
                {show.ChqDt && (
                    <>
                        <Text style={styles.label}>Cheque / Payment Date (YYYY-MM-DD)</Text>
                        <TextInput
                            style={styles.input}
    ...
    return (
        <ScrollView style={styles.container} contentContainerStyle={styles.formContent}>
            ...
            <View style={styles.reviewCard}>
                ...
                <Text style={styles.reviewRow}>
                    Vehicles selected:{' '}
                    {vehicleList.filter(v => v.IsChecked).length}
                </Text>
                <Text style={styles.reviewRow}>
                    Epayments selected:{' '}
                    {epaymentList.filter(e => e.IsChecked).length}
                </Text>
            </View>
