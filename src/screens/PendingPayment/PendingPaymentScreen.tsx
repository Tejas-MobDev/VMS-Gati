...
const PendingPaymentScreen = () => {
    ...
    const handleSearch = (text: string) => {
        ...
        if (!text) {
            ...
            setTotalVehicleBalance(filtered.reduce((s, c) => s + (c.Balance ?? 0), 0));
        }
    };

    const saveRemark = async (item: any, type: 'Vehicle' | 'Epayment') => {
        const key = `${type}_${item.ID ?? item.SalesOrderDetID}`;
        console.log('[PendingPayment] Save Remark pressed | type:', type, '| key:', key);
        const remark = remarks[key] ?? item.PaymentRemark ?? '';
        const paymentData = {
            SalesOrderdetID:
                type === 'Epayment' ? item.SalesOrderDetID : item.ID,
            RemarkPayment: remark,
            RemarkType: type,
        };
        setIsLoading(true);
        try {
            const res = await SavePaymentRemarkInSalesOrderDet(paymentData, sessionToken!);
            HelperService.showAlert('', res.Msg);
        } catch {
        ...
        return (
            <View style={[styles.card, isOld && styles.cardOld]}>
                ...
                <Text style={styles.detail}>
                    Allotted on: {dateTimeSplit(item.AllotmentDateOn)}. Aging: {item.Aging}
                </Text>
                <Text style={styles.detail}>
...
const styles = StyleSheet.create({
    ...
    totalText: { fontWeight: '700', color: '#333', fontSize: 14 },
    ...
    vendorName: { fontWeight: '700', fontSize: 14, color: '#222', marginBottom: 4 },
