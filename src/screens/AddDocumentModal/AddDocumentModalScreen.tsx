...
const AddDocumentModalScreen = () => {
    ...
    const { compulsoryDocIDs = [], maxLengthDocIDList = [], returnScreen = 'AttachNewDocs' } =
        route.params ?? {};
    ...
    useEffect(() => {
        Promise.all([
            ...
            if (typeRes.IsSuccess) { setDocTypeList(typeRes.Data ?? []); }
            if (subRes.IsSuccess) { setAllSubDocList(subRes.Data ?? []); }
        });
    }, []);
        ...
        launchImageLibrary(
            ...
            response => {
                ...
                if (response.errorCode) {
                    HelperService.showAlert('Error', response.errorMessage ?? 'File picker error.');
                    return;
                }
    ...
    const handleSave = () => {
        console.log('[AddDocumentModal] Save button pressed | data:', JSON.stringify(newDocData));
        let errorText = '';
        ...
        // Pass data back to AttachNewDocs via navigation params
        navigation.navigate(returnScreen, { newDoc: newDocData });
    };
...
const styles = StyleSheet.create({
    ...
    content: { padding: 16 },
    label: { fontSize: 13, color: '#555', fontWeight: '600', marginBottom: 4, marginTop: 14 },
    pickerWrapper: {
    ...
    uploadBtn: {
        borderWidth: 1, borderColor: '#3880ff', borderRadius: 8,
    ...
    previewImage: { width: 120, height: 120, borderRadius: 8, backgroundColor: '#eee' },
    previewLabel: { color: '#2dd36f', fontWeight: '600', marginTop: 6 },
    loader: { marginVertical: 12 },
    btnRow: { flexDirection: 'row', gap: 12, marginTop: 24, marginBottom: 32 },
    cancelBtn: { flex: 1, backgroundColor: '#92949c', borderRadius: 8, paddingVertical: 12, alignItems: 'center' },
    saveBtn: { flex: 1, backgroundColor: '#3880ff', borderRadius: 8, paddingVertical: 12, alignItems: 'center' },
